            ...
            with open(os.path.join(self.data_dir, "localisation", language, "scene.csv"),
                ...
                for index, row in enumerate(rd[1:]):
                    if row[0]:  # chapter, no need for else since must have chapter
                        if row[0] not in self.text[language]["scene"]:
                            self.text[language]["scene"][row[0]] = {}
                        if row[1]:  # mission
                            if row[1] not in self.text[language]["scene"][row[0]]:
                                self.text[language]["scene"][row[0]][row[1]] = {}
                            if row[2]:  # scene
                                if row[2] not in self.text[language]["scene"][row[0]][row[1]]:
                                    self.text[language]["scene"][row[0]][row[1]][row[2]] = {}
                                if row[3]:  # scene
                                    if row[3] not in self.text[language]["scene"][row[0]][row[1]][row[2]]:
                                        # last at scene
                                        self.text[language]["scene"][row[0]][row[1]][row[2]][row[3]] = {"Text": row[4]}
                                else:
                                    self.text[language]["scene"][row[0]][row[1]][row[2]]["Text"] = row[4]
                            else:
                                self.text[language]["scene"][row[0]][row[1]]["Text"] = row[4]
                        else:
                            self.text[language]["scene"][row[0]]["Text"] = row[4]
            edit_file.close()
        except FileNotFoundError:
            pass
