...
class Effect(Sprite):
    ...
    def __init__(self, battle_camera, owner, part_stat, layer, moveset=None, from_owner=True):
        ...
        if self.reach_effect:
            ...
            for _ in range(spawn_number):
                ...
                if "no dmg" in self.effect_stat["Property"]:
                    Effect(self.battle_camera, self.owner, stat, layer, moveset=self.moveset, from_owner=False)
                else:
                    DamageEffect(self.battle_camera, self.owner, stat, layer, moveset=self.moveset, from_owner=False)
            ...
            if self.x_momentum or self.y_momentum:  # sprite bounce after reach
                ...
                if self.stuck_part:  # find new pos based on stuck part
                    if self.stuck_part.data:
                        ...
                        # else:
                        self.pos[0] += x_diff * scale_diff[0]
